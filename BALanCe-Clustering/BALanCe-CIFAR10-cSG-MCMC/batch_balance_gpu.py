...
def choose_subset(pool_pred_lt1, pool_pred_lt2, hamming_dis_lt,  
                origin_pos_lt, subset_size, device, 
                M=10000, sample_num=20, sampling_index=3, score_batch_size=20):
    ...
    pool_size = pool_pred_lt1.shape[0]
    pos_lt = []
...
def acquire(model, pool, hamming_pool, device, B=10, M=10000, sample_num=20, sampling_index=3, hamming_dis_threshold=0.05, score_batch_size=20):
    model.eval()
    ...
    for i in range(len(acquire_num_lt)):
        ...
        pred_lt1 = pool_pred_lt1[index_lt]
        pred_lt2 = pool_pred_lt2[index_lt]
        sub_pos_lt = choose_subset(pred_lt1, pred_lt2, hamming_dis_lt, pos_lt, subset_size, 
                device=device, M=M, sample_num=sample_num, sampling_index=sampling_index, score_batch_size=score_batch_size)
        choose_pos_lt += sub_pos_lt
        #print(f'sub_pos_lt: {len(sub_pos_lt)}')
