...
def one_pass(pass_time):
    ...
    while len(buff_train.labels) < 600:
        ...
        _, pos_lt = mc_eced_gpu.delta_batch_mc_eced(net, pool, buff_hamming, M=M, B=B, sampling_index=sampling_index, device=device, sample_num=sample_num, hamming_dis_threshold=(1.0-val_acc)/2.0)
        feat_lt, label_lt = pool.query_labels(pos_lt)
        buff_train.add_rows(feat_lt, label_lt)
        print(f'ECED Anneal MC Batch Pass:{pass_time}, train_size:{len(dataset_train.labels)}, test_acc:{test_acc}, pos_lt: {pos_lt} ')
...
for pass_time in range(al_num):
    ...
    np.savetxt(f'ECED-MC-Batch_sample-num-{sample_num}_hamming-anneal_B-{B}_M-{M}_patience-{patience}.out', acc_lts_np, delimiter=',')
