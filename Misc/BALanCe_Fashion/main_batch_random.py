...
def train_eval_model(dataloader_train, dataloader_val, dataloader_test, weight):
    ...
    for epoch in range(500):
        ...
        for b_id, (x, label) in enumerate(dataloader_train):
            ...
            pred = train_net(x)
            label = label.view(-1)
            loss = F.nll_loss(pred, label, weight)
            loss.backward()
            opt.step()
...
def one_pass(pass_time):
    ...
    test_acc_lt = []
    while len(buff_train.labels) < 360:
        dataset_train = MyDataset(buff_train)
        weight = dataset_train.get_weight()
        if len(buff_train.labels) < 360:
            sampler = torch.utils.data.RandomSampler(dataset_train, replacement=True, num_samples=20224)
        ...
        buff_train.add_rows(feat_lt, label_lt)
        print(f'Batch Random Pass:{pass_time}, train_size:{ len(buff_train.labels)}, test_acc:{test_acc} ')
...
for pass_time in range(al_num):
    ...
    np.savetxt(f'Random-batch_B-{B}_patience-{patience}.out', acc_lts_np, delimiter=',')
