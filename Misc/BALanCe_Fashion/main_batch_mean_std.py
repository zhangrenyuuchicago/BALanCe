...
def one_pass(pass_time):
    ...
    while len(buff_train.labels) < 360:
        ...
        buff_train.add_rows(feat_lt, label_lt)
        print(f'Mean STD Pass:{pass_time}, train_size:{len(dataset_train.labels)}, test_acc:{test_acc}, pos_lt: {pos_lt} ')
...
for pass_time in range(al_num):
    ...
    np.savetxt(f'Mean-STD-Batch_sample-num-{sample_num}_B-{B}_patience-{patience}.out', acc_lts_np, delimiter=',')
