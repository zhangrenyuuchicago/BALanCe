...
class Pool():
    ...
    def repeat(self):
        num_repetitions=3
        features_cuda = torch.from_numpy(self.features)
        labels_cuda = torch.from_numpy(self.labels)

        dataset_noise = torch.empty(
                                (len(self.features) * num_repetitions, 28, 28), dtype=torch.float32
                        ).normal_(0.0, 0.03)

        features_cuda = torch.cat([features_cuda]*num_repetitions, dim=0)
        labels_cuda = torch.cat([labels_cuda]*num_repetitions, dim=0)
        features_cuda = features_cuda + dataset_noise*255
        features_cuda[features_cuda > 255] = 255
        features_cuda[features_cuda < 0] = 0
        features_cuda = features_cuda.to(torch.uint8)

        self.features = features_cuda.cpu().numpy()
        self.labels = labels_cuda.cpu().numpy()

        '''
        self.features = np.concatenate((self.features, self.features, self.features), axis=0)
        self.labels = np.concatenate((self.labels, self.labels, self.labels), axis=0)
