...
def train_eval_model(dataloader_train, dataloader_val, dataloader_test, weight):
    ...
    net = vgg_model.vgg11(num_classes=10).to(device)
...
def one_pass(pass_time):
    ...
    while len(buff_train.labels) < 1405:
        ...
        buff_train.add_rows(feat_lt, label_lt)
        print(f'Random Batch Pass:{pass_time}, train_size:{len(dataset_train.labels)}, test_acc:{test_acc} ')
...
np.savetxt(f'Random-Batch_sample-num-{sample_num}_B-{B}_patience-{patience}.out', acc_lts, delimiter=',')
