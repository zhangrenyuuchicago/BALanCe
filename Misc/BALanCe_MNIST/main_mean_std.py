...
def one_pass(pass_time):
    ...
    while len(buff_train.labels) < 260:
        ...
        _, pos = mean_std.delta_mean_std(net, pool, device=device, sample_num=sample_num)
        feat, label = pool.query_label(pos)
        buff_train.add_row(feat, label)
        print(f'Mean STD Pass:{pass_time}, train_size:{len(buff_train.labels)}, test_acc:{test_acc}, pos: {pos} ')
...
np.savetxt(f'Mean-STD_sample-num-{sample_num}-patience-{patience}.out', acc_lts, delimiter=',')
