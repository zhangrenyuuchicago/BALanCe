...
def one_pass(pass_time):
    ...
    while len(buff_train.labels) < 200:
        ...
        _, pos_lt = mc_mis_gpu.delta_mis_batch_mc(net, pool, device=device, M=M, B=B, sampling_index=sampling_index, sample_num=sample_num)
        feat_lt, label_lt = pool.query_labels(pos_lt)
        buff_train.add_rows(feat_lt, label_lt)
        print(f'MIS Batch MC Pass:{pass_time}, train_size:{len(dataset_train.labels)}, test_acc:{test_acc}, pos_lt: {pos_lt}, test_auc: {test_auc} ')
...
for pass_time in range(al_num):
    ...
    np.savetxt(f'MIS-MC-Batch_sample-num-{sample_num}_B-{B}_M-{M}-patience-{patience}.acc.out', acc_lts_np, delimiter=',')

    auc_lts_np = np.array(auc_lts)
    np.savetxt(f'MIS-MC-Batch_sample-num-{sample_num}_B-{B}_M-{M}-patience-{patience}.auc.out', auc_lts_np, delimiter=',')
