...
def train_eval_model(dataloader_train, dataloader_val, dataloader_test, weight):
    net = model.BayesianNet(7).to(device)
...
def one_pass(pass_time):
    ...
    pool = Pool.Pool('./data/DryBeanDataset/training.pt')
    pool_test = Pool.Pool('./data/DryBeanDataset/test.pt')
    pool_hamming = Pool.Pool('./data/DryBeanDataset/hamming.pt')

    #feat_lt, label_lt = pool.random_query_labels(val_dataset_size)
    #buff_val.add_rows(feat_lt, label_lt)
    buff_val.add_from_pt('./data/DryBeanDataset/val.pt')

    #feat_lt, label_lt = pool.random_query_balanced_labels(K=2)
    #buff_train.add_rows(feat_lt, label_lt)
    buff_train.add_from_pt('./data/DryBeanDataset/start.pt')

    #feat_lt, label_lt = pool.random_query_labels(hamming_pool_size)
    #buff_hamming.add_rows(feat_lt, label_lt)
    #buff_hamming.add_from_pt('./data/DryBeanDataset/hamming.pt')
    ...
    while len(buff_train.labels) < 200:
        ...
        sampler = torch.utils.data.RandomSampler(dataset_train, replacement=True, num_samples=8192)
        ...
        _, pos_lt = mc_eced_gpu.delta_batch_mc_eced(net, pool, pool_hamming, M=M, B=B, sampling_index=sampling_index, device=device, sample_num=sample_num, hamming_dis_threshold=(1.0-val_acc)/4.0)
        feat_lt, label_lt = pool.query_labels(pos_lt)
        buff_train.add_rows(feat_lt, label_lt)
        print(f'ECED Anneal MC Batch Pass:{pass_time}, train_size:{len(dataset_train.labels)}, test_acc:{test_acc}, test_auc:{test_auc}, pos_lt: {pos_lt} ')
...
for pass_time in range(al_num):
    ...
    np.savetxt(f'ECED-MC-Batch_sample-num-{sample_num}_hamming-anneal_B-{B}_M-{M}_patience-{patience}.acc.out', acc_lts_np, delimiter=',')

    auc_lts_np = np.array(auc_lts)
    np.savetxt(f'ECED-MC-Batch_sample-num-{sample_num}_hamming-anneal_B-{B}_M-{M}_patience-{patience}.auc.out', auc_lts_np, delimiter=',')
