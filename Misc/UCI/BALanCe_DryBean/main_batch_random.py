...
def one_pass(pass_time):
    ...
    #feat_lt, label_lt = pool.random_query_labels(250)
    ...
    while len(buff_train.labels) < 200:
        ...
        #_, pos_lt = mc_eced.delta_batch_mc_eced(net, pool, buff_hamming, M=M, B=B, device=device, sample_num=sample_num, hamming_dis_threshold=hamming_dis_threshold)
        #feat_lt, label_lt = pool.query_labels(pos_lt)
        feat_lt, label_lt = pool.random_query_labels(B)
        
        buff_train.add_rows(feat_lt, label_lt)
        print(f'Random Pass:{pass_time}, train_size:{ len(buff_train.labels)}, test_acc:{test_acc}, test_auc:{test_auc} ')
...
for pass_time in range(al_num):
    ...
    np.savetxt(f'Random-batch_B-{B}_patience-{patience}.acc.out', acc_lts_np, delimiter=',')

    auc_lts_np = np.array(auc_lts)
    np.savetxt(f'Random-batch_B-{B}_patience-{patience}.auc.out', auc_lts_np, delimiter=',')
