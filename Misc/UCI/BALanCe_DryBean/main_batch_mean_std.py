...
def one_pass(pass_time):
    ...
    while len(buff_train.labels) < 200:
        ...
        _, pos_lt = mean_std.delta_mean_std_batch(net, pool, B=B, device=device, sample_num=sample_num)
        feat_lt, label_lt = pool.query_labels(pos_lt)
        buff_train.add_rows(feat_lt, label_lt)
        print(f'Mean STD Batch Pass:{pass_time}, train_size:{len(dataset_train.labels)}, test_acc:{test_acc}, test_auc:{test_auc}, pos_lt: {pos_lt} ')
...
for pass_time in range(al_num):
    ...
    np.savetxt(f'Mean-STD-Batch_sample-num-{sample_num}_B-{B}_patience-{patience}.acc.out', acc_lts_np, delimiter=',')

    auc_lts_np = np.array(auc_lts)
    np.savetxt(f'Mean-STD-Batch_sample-num-{sample_num}_B-{B}_patience-{patience}.auc.out', auc_lts_np, delimiter=',')
