...
def one_pass(pass_time):
    ...
    while len(buff_train.labels) < 50:
        ...
        _, pos = variation_ratios.delta_variation_ratios(net, pool, device=device, sample_num=sample_num)
        feat, label = pool.query_label(pos)
        buff_train.add_row(feat, label)
        print(f'Variation Ratio Pass:{pass_time}, train_size:{len(buff_train.labels)}, test_acc:{test_acc}, pos: {pos} ')
...
np.savetxt(f'Variation-ratio_sample-num-{sample_num}-patience-{patience}.out', acc_lts, delimiter=',')
