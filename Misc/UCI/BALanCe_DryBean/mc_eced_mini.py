...
def delta_batch_mc_eced(model, pool, hamming_pool, device, B=10, M=100, sample_num=20, sampling_index=3, hamming_dis_threshold=0.05, class_num=10):
    ...
    for b in range(sampling_index, B):
        ...
        acq_sample = []
        mini_start = 0
        while mini_start < pool_size:
            if mini_start + mini_batch_size < pool_size:
                mini_end = mini_start + mini_batch_size
            else:
                mini_end = pool_size

            max_P_g_mini = np.amax(pool_pred_lt1[mini_start:mini_end], axis=2) * max_P_g
            max_P_g_prime_mini = np.amax(pool_pred_lt2[mini_start:mini_end], axis=2) * max_P_g_prime
        
            P_comb_n_1 = np.concatenate((P_g_n_1, P_g_prime_n_1), axis=0)
            # sample*m*c
            P_comb_n_1_P_n_mini = np.matmul(np.swapaxes(pool_pred_comb_lt[mini_start:mini_end], 1, 2), P_comb_n_1)/sample_num
            ...
            # sample*k*m*c
            P_g_n_mini = np.matmul(P_g_n_1[np.newaxis,:,:,np.newaxis], pool_pred_lt1[mini_start:mini_end,:,np.newaxis,:])
            P_g_prime_n_mini = np.matmul(P_g_prime_n_1[np.newaxis,:,:,np.newaxis], pool_pred_lt2[mini_start:mini_end,:,np.newaxis,:])
            ...
            acq_sample_mini = np.sum(P_comb_n_1_div_P_n_1_sum_mini*edge_cut_mini, axis=(1,2))/(10**sampling_index)
            acq_sample += list(acq_sample_mini)
            #print(f'mini_start:{mini_start}, acq_sample: {acq_sample}') 
            mini_start += mini_batch_size
