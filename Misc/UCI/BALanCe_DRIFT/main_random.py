...
def train_eval_model(dataloader_train, dataloader_val, dataloader_test, weight):
    ...
    net = model.BayesianNet(2).to(device)
    opt = optim.Adam(list(net.parameters()), lr=0.01)
    ...
    for epoch in range(500):
        ...
        if acc <= best_acc:
            if epoch - best_epoch > patience:
                print(f'patience ex; best epoch: {best_epoch}; best acc: {best_acc}')
                break
        else:
            best_acc = acc
            best_epoch = epoch
...
def one_pass(pass_time):
    ...
    pool = Pool.Pool('./data/Telescope/training.pt')
    pool_test = Pool.Pool('./data/Telescope/test.pt')
    
    #feat_lt, label_lt = pool.random_query_labels(val_dataset_size)
    #buff_val.add_rows(feat_lt, label_lt)
    buff_val.add_from_pt('./data/Telescope/val.pt')

    #feat_lt, label_lt = pool.random_query_balanced_labels(K=2)
    #buff_train.add_rows(feat_lt, label_lt)
    buff_train.add_from_pt('./data/Telescope/start.pt')

    #feat_lt, label_lt = pool.random_query_labels(hamming_pool_size)
    #buff_hamming.add_row(feat_lt, label_lt)
    buff_hamming.add_from_pt('./data/Telescope/hamming.pt')
    ...
    test_acc_lt = []
    while len(buff_train.labels) < 50000:
        dataset_train = MyDataset(buff_train) 
        weight = dataset_train.get_weight()
        if len(buff_train.labels) < 50000:
            sampler = torch.utils.data.RandomSampler(dataset_train, replacement=True, num_samples=1024)
        ...
        net, val_acc, test_acc = train_eval_model(dataloader_train, dataloader_val, dataloader_test, weight)
        
        feat, label = pool.random_query_label()
        buff_train.add_row(feat, label)
        print(f'Random Pass:{pass_time}, train_size:{len(buff_train.labels)}, test_acc:{test_acc} ')
...
np.savetxt(f'Random_patience-{patience}.out', acc_lts, delimiter=',')
