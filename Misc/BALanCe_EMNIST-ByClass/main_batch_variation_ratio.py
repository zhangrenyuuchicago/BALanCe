...
def one_pass(pass_time):
    ...
    while len(buff_train.labels) < 450:
        ...
        _, pos_lt = variation_ratios.delta_variation_ratios_batch(net, pool, device=device, B=B, sample_num=sample_num)
        feat_lt, label_lt = pool.query_labels(pos_lt)
        buff_train.add_rows(feat_lt, label_lt)
        print(f'Variation Ratio Batch Pass:{pass_time}, train_size:{len(dataset_train.labels)}, test_acc:{test_acc}, pos_lt: {pos_lt} ')
...
for pass_time in range(al_num):
    ...
    np.savetxt(f'Variation-Ratio-Batch_sample-num-{sample_num}_B-{B}_patience-{patience}.out', acc_lts_np, delimiter=',')
