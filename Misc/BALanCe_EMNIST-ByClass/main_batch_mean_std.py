...
def one_pass(pass_time):
    ...
    while len(buff_train.labels) < 450:
        ...
        _, pos_lt = mean_std.delta_mean_std_batch(net, pool, device=device, B=B, sample_num=sample_num)
        feat_lt, label_lt = pool.query_labels(pos_lt)
        buff_train.add_rows(feat_lt, label_lt)
        print(f'Mean STD Batch Pass:{pass_time}, train_size:{len(dataset_train.labels)}, test_acc:{test_acc}, pos_lt: {pos_lt} ')
