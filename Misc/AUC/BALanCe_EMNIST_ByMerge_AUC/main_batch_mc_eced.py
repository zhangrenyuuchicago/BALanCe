...
def one_pass(pass_time):
    ...
    while len(buff_train.labels) < 600:
        ...
        buff_train.add_rows(feat_lt, label_lt)
        print(f'ECED MC Batch Pass:{pass_time}, train_size:{len(dataset_train.labels)}, test_acc:{test_acc}, test_auc:{test_auc}, pos_lt: {pos_lt} ')
...
for pass_time in range(al_num):
    ...
    np.savetxt(f'ECED-MC-Batch_sample-num-{sample_num}_hamming-{hamming_dis_threshold}_B-{B}_M-{M}_patience-{patience}.acc.out', acc_lts_np, delimiter=',')

    auc_lts_np = np.array(auc_lts)
    np.savetxt(f'ECED-MC-Batch_sample-num-{sample_num}_hamming-{hamming_dis_threshold}_B-{B}_M-{M}_patience-{patience}.auc.out', auc_lts_np, delimiter=',')
