...
def one_pass(pass_time):
    ...
    pool_test = Pool.Pool('./data/EMNIST/fixed/test_bymerge.pt')
    ...
    test_acc_lt = []
    test_auc_lt = []
    while len(buff_train.labels) < 600:
        ...
        _, pos_lt = mc_eced_gpu.delta_batch_mc_eced(net, pool, buff_hamming, M=M, B=B, sampling_index=sampling_index, device=device, sample_num=sample_num, hamming_dis_threshold=(1.0-val_acc)/4.0)
