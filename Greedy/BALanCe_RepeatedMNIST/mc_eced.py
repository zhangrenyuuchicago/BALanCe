...
def delta_batch_mc_eced(model, pool, hamming_pool, device, B=10, M=100, sample_num=20, sampling_index=3, hamming_dis_threshold=0.05, class_num=10):
    ...
    for b in range(sampling_index, B):
        ...
        # sample*k*m*c
        P_g_n = np.matmul(P_g_n_1[np.newaxis,:,:,np.newaxis], pool_pred_lt1[:,:,np.newaxis,:])
        P_g_prime_n = np.matmul(P_g_prime_n_1[np.newaxis,:,:,np.newaxis], pool_pred_lt2[:,:,np.newaxis,:])
        discount = 1.0 - (P_g_n/(max_P_g[:,:,None,None]))*(P_g_prime_n/(max_P_g_prime[:,:,None,None]))
        # sample*m*c
        edge_cut = np.sum(discount*hamming_dis_lt[np.newaxis,:,None,None], axis=1)
        acq_sample = np.sum(P_comb_n_1_div_P_n_1_sum*edge_cut, axis=(1,2))
