...
def one_pass(pass_time):
    ...
    while len(buff_train.labels) < 260:
        ...
        _, pos_lt = eced.delta_batch_eced(net, pool, buff_hamming, B=B, device=device, sample_num=sample_num, hamming_dis_threshold=hamming_dis_threshold)
...
np.savetxt(f'ECED-batch_sample-num_{sample_num}_hamming-{hamming_dis_threshold}_B-{B}_patience-{patience}.out', acc_lts, delimiter=',')
