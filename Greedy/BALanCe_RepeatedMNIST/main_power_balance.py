...
def one_pass(pass_time):
    ...
    while len(buff_train.labels) < 300:
        ...
        pos_lt = power_balance.power_balance(net, pool, buff_hamming, B=B, device=device, sample_num=sample_num, hamming_dis_threshold=hamming_dis_threshold)
        feat_lt, label_lt = pool.query_labels(pos_lt)
        buff_train.add_rows(feat_lt, label_lt)
        print(f'PowerBALanCe Pass:{pass_time}, train_size:{len(dataset_train.labels)}, test_acc:{test_acc}, pos_lt: {pos_lt} ')
...
for pass_time in range(al_num):
    ...
    np.savetxt(f'PowerBALanCe_sample-num-{sample_num}_hamming-anneal-div4_B-{B}_M-{M}_patience-{patience}.out', acc_lts_np, delimiter=',')
