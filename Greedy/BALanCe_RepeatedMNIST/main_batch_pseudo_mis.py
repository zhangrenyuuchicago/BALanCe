...
def one_pass(pass_time):
    ...
    while len(buff_train.labels) < 300:
        ...
        _, pos_lt = mis.delta_pseudo_batch_mis(net, pool, device=device, B=B, sample_num=sample_num)
        feat_lt, label_lt = pool.query_labels(pos_lt)
        buff_train.add_rows(feat_lt, label_lt)
        print(f'MIS Pseudo Pass:{pass_time}, train_size:{len(dataset_train.labels)}, test_acc:{test_acc}, pos_lt: {pos_lt} ')
...
np.savetxt(f'MIS-Pseudo_sample-num-{sample_num}_B-{B}_patience-{patience}.out', acc_lts, delimiter=',')
