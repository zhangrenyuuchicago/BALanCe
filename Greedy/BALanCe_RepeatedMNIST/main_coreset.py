...
def one_pass(pass_time):
    ...
    while len(buff_train.labels) < 300:
        ...
        _, pos_lt = coreset.delta_coreset(net, pool, buff_train, device=device, M=M, B=B, sampling_index=sampling_index, sample_num=sample_num)
        feat_lt, label_lt = pool.query_labels(pos_lt)
        buff_train.add_rows(feat_lt, label_lt)
        print(f'CoreSet Pass:{pass_time}, train_size:{len(dataset_train.labels)}, test_acc:{test_acc}, pos_lt: {pos_lt} ')
...
for pass_time in range(al_num):
    ...
    np.savetxt(f'CoreSet-Batch_sample-num-{sample_num}_B-{B}_M-{M}-patience-{patience}.out', acc_lts_np, delimiter=',')
