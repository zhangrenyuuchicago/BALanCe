...
def one_pass(pass_time):
    ...
    pool = Pool.Pool('./data/MNIST/fixed/training_repeat.pt')
    ...
    while len(buff_train.labels) < 300:
        ...
        pos_lt = power_bald.power_bald(net, pool, device=device, B=B, sample_num=sample_num)
        feat_lt, label_lt = pool.query_labels(pos_lt)
        buff_train.add_rows(feat_lt, label_lt)
        print(f'PowerBALD Pass:{pass_time}, train_size:{len(dataset_train.labels)}, test_acc:{test_acc}, pos_lt: {pos_lt} ')
...
for pass_time in range(al_num):
    ...
    np.savetxt(f'PowerBALD_sample-num-{sample_num}_B-{B}_M-{M}-patience-{patience}.out', acc_lts_np, delimiter=',')
